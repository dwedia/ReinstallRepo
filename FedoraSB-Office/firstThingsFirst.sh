...
if [ "$ostreeStatus" == "idle" ]; then
    # Install packages
    rpm-ostree install i3 rofi ansible-core distrobox terminator vim lxpolkit https://mirrors.rpmfusion.org/free/fedora/rpmfusion-free-release-$(rpm -E %fedora).noarch.rpm https://mirrors.rpmfusion.org/nonfree/fedora/rpmfusion-nonfree-release-$(rpm -E %fedora).noarch.rpm
